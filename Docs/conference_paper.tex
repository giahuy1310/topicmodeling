...
ensemble against (i)~two in-house fusion baselines that use the same five
out-of-fold class-probability vectors and the same leakage-controlled protocol, but
do not observe the original sentence: weighted soft voting (accuracy-normalized
convex combination of the five probability vectors, then $\arg\max$) and a
multinomial logistic-regression stacking classifier trained on the concatenated
35-dimensional OOF probability vector, with the regularization parameter $C$
selected on validation macro-F1; (ii)~Gemma-2-9B-it zero-shot prompting on the same
structured meta-prompts; (iii)~published monolingual and multilingual transformer
results on UIT-VSMEC (CafeBERT and XLM-RoBERTa-Large from VLUE \cite{do2024vlue},
ViSoBERT \cite{nguyen2024visobert}); (iv)~the MLR pipeline with preprocessing and
key-clause extraction \cite{nguyen2020exploiting}; and (v)~the GRU--CNN--BiLSTM--LSTM
neural ensemble \cite{huynh2020ensemble}. For context, the original UIT-VSMEC
benchmark CNN+word2vec baseline \cite{ho2020emotion} reached 59.74\% weighted F1 but
is omitted from Table~\ref{tab:ensemble} because it predates the transformer-centric
comparison protocol used in our evaluation.
...
Weighted soft voting (ours)               & --     & --     & -- \\
LogReg stacking meta (ours)               & --     & --     & -- \\
...
\bottomrule
\multicolumn{4}{@{}l}{\footnotesize W-F1: weighted F1;\enspace M-F1: macro F1;\enspace ---: not reported;\enspace --: fill from notebook 07 \texttt{ensemble\_summary.json}.} \\
\end{tabular}
\end{table}
...
The two in-house fusion rows isolate the contribution of the LLM meta-learner from
ordinary stacking. Weighted soft voting tests \emph{fixed} fusion: the same
accuracy-normalized mix already provided to Gemma as a consensus summary. LogReg
stacking tests a \emph{trainable linear} meta-classifier on the concatenated
probability vectors alone (no sentence). Zero-shot Gemma tests the LLM without SFT.
The proposed QLoRA meta-model is justified only if it outperforms both fusion
baselines on weighted F1; that gap, if present, is attributable to
text-conditioned, instance-specific fusion rather than to stacking in general.
If it merely matches LogReg stacking, a linear meta-head on probability vectors is
sufficient and the LLM is not justified. Fill the two `--` rows from notebook 07
before claiming this gap.

A 2.13 percentage-point gain in weighted F1 over Gemma zero-shot (70.22\% vs 68.09\%)
shows that QLoRA fine-tuning on the structured dataset helps relative to zero-shot
inference, but does not by itself show that Gemma is a better meta-learner than
LogReg stacking. The best augmented single model (CafeBERT, 67.10\% W-F1) trails the
full ensemble by 3.1 percentage points in weighted F1 (67.10\% vs 70.22\%), showing
that the stacked system integrates heterogeneous BERT and TF-IDF signals beyond any
...
The full ensemble surpassed zero-shot Gemma prompting by 2.13 percentage points in
weighted F1 and the best augmented single model (CafeBERT) by approximately 3.1
percentage points in weighted F1. Whether QLoRA meta-learning is warranted beyond
ordinary stacking is settled by the two in-house fusion baselines---weighted soft
voting and LogReg stacking on the same probability vectors---which share the OOF
protocol but never see the sentence. Together, these results confirm that LLMs are
effective as offline data generators; their role as inference-time meta-learners
holds only if the QLoRA model outperforms those fusion baselines.
